...
final class DEWeekParsingTests: XCTestCase {
    private let referenceDate = Date(timeIntervalSince1970: 1736899200) // 2025-01-15
    private let calendar = Calendar(identifier: .iso8601)

    func testDEISOWeekParsing() {
        let results = Chrono.de.casual.parse(text: "Woche 15 2023", referenceDate: referenceDate)
    ...
    func testDERelativeWeekParsing() {
        let results = Chrono.de.casual.parse(text: "nächste Woche", referenceDate: referenceDate)
    ...
    func testDEWeeksAgoParsing() {
        let results = Chrono.de.casual.parse(text: "vor 2 Wochen", referenceDate: referenceDate)
        guard let weekResult = results.first(where: { $0.start.get(.isoWeek) != nil }) else {
            XCTFail("No relative week parsing result found")
            return
        }
