...
final class FRWeekParsingTests: XCTestCase {
    private let referenceDate = Date(timeIntervalSince1970: 1736899200) // 2025-01-15
    private let calendar = Calendar(identifier: .iso8601)

    func testFRISOWeekParsing() {
        let results = Chrono.fr.casual.parse(text: "semaine 15 2023", referenceDate: referenceDate)
    ...
    func testFRRelativeWeekParsing() {
        let results = Chrono.fr.casual.parse(text: "la semaine prochaine", referenceDate: referenceDate)
    ...
    func testFRWeeksAgoParsing() {
        let results = Chrono.fr.casual.parse(text: "il y a 2 semaines", referenceDate: referenceDate)
        guard let weekResult = results.first(where: { $0.start.get(.isoWeek) != nil }) else {
            XCTFail("No relative week parsing result found")
            return
        }
