...
final class ESWeekParsingTests: XCTestCase {
    private let referenceDate = Date(timeIntervalSince1970: 1736899200) // 2025-01-15
    private let calendar = Calendar(identifier: .iso8601)

    func testESISOWeekParsing() {
        let results = Chrono.es.casual.parse(text: "semana 15 de 2023", referenceDate: referenceDate)
    ...
    func testESRelativeWeekParsing() {
        let results = Chrono.es.casual.parse(text: "la próxima semana", referenceDate: referenceDate)
    ...
    func testESWeeksAgoParsing() {
        let results = Chrono.es.casual.parse(text: "hace 2 semanas", referenceDate: referenceDate)
        guard let weekResult = results.first(where: { $0.start.get(.isoWeek) != nil }) else {
            XCTFail("No relative week parsing result found")
            return
        }
