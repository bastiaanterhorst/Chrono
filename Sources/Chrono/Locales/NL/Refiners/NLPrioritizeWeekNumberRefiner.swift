// NLPrioritizeWeekNumberRefiner.swift - Prioritize Dutch week-based parser results
import Foundation

/// Prioritizes Dutch week parser results over conflicting results at the same index
final class NLPrioritizeWeekNumberRefiner: Refiner, @unchecked Sendable {
    func refine(context: ParsingContext, results: [ParsingResult]) -> [ParsingResult] {
        ...
        for (_, indexedResults) in groupedByIndex {
            ...
            let weekResults = indexedResults.filter { result in
                result.hasTag("NLISOWeekParser") || result.hasTag("NLRelativeWeekParser")
            }
