// PTPrioritizeWeekNumberRefiner.swift - Prioritize Portuguese week-based parser results
import Foundation

/// Prioritizes Portuguese week parser results over conflicting results at the same index
final class PTPrioritizeWeekNumberRefiner: Refiner, @unchecked Sendable {
    func refine(context: ParsingContext, results: [ParsingResult]) -> [ParsingResult] {
        ...
        for (_, indexedResults) in groupedByIndex {
            ...
            let weekResults = indexedResults.filter { result in
                result.hasTag("PTISOWeekParser") || result.hasTag("PTRelativeWeekParser")
            }
