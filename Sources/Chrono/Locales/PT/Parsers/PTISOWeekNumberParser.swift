...
final class PTISOWeekNumberParser: AbstractParserWithWordBoundaryChecking, @unchecked Sendable {
    override func innerPattern(context: ParsingContext) -> String {
        let weekPattern = "(?i)(?:\\b(?:semana|sem)\\b\\s*(?:n[uú]mero\\s*)?(?:#\\s*)?\\d{1,2}(?:[ªaºo])?(?:\\s*(?:de|da|do|em)?\\s*(?:'\\d{2}|\\d{2}|\\d{4}))?)"
        let ordinalWeekPattern = "(?i)(?:\\b(?:a\\s+)?\\d{1,2}(?:[ªaºo]|\\.)\\s+semana(?:\\s+de\\s+(?:'\\d{2}|\\d{2}|\\d{4}))?)"
