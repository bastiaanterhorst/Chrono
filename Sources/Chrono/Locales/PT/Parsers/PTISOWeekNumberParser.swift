// PTISOWeekNumberParser.swift - Parser for ISO week numbers in Portuguese text
import Foundation

/// Parser for ISO 8601 week numbers in Portuguese text
final class PTISOWeekNumberParser: AbstractParserWithWordBoundaryChecking, @unchecked Sendable {
    override func innerPattern(context: ParsingContext) -> String {
        let weekPattern = "(?i)(?:\\b(?:semana|sem)\\b\\s*(?:n[uú]mero\\s*)?(?:#\\s*)?\\d{1,2}(?:[ªaºo])?(?:\\s*(?:de|da|do|em)?\\s*(?:\\d{4}|'\\d{2}|\\d{2}))?)"
        let ordinalWeekPattern = "(?i)(?:\\b(?:a\\s+)?\\d{1,2}(?:[ªaºo]|\\.)\\s+semana(?:\\s+de\\s+(?:\\d{4}|'\\d{2}|\\d{2}))?)"
    ...
    private func extractWeekNumber(from text: String) -> Int? {
        ...
        if let groups = captureGroups(
            pattern: "(?i)^(?:semana|sem)\\s*(?:n[uú]mero\\s*)?(?:#\\s*)?(\\d{1,2})(?:[ªaºo])?(?:\\s*(?:de|da|do|em)?\\s*(?:\\d{4}|'\\d{2}|\\d{2}))?$",
            text: text
        ), groups.count >= 2, let week = Int(groups[1]) {
            return week
        }
        ...
        if let groups = captureGroups(
            pattern: "(?i)^(?:a\\s+)?(\\d{1,2})(?:[ªaºo]|\\.)\\s+semana(?:\\s+de\\s+(?:\\d{4}|'\\d{2}|\\d{2}))?$",
            text: text
        ), groups.count >= 2, let week = Int(groups[1]) {
            return week
        }
        ...
        if text.lowercased().contains("semana") || text.lowercased().contains("sem") || text.lowercased().contains("w") {
    ...
    private func extractWeekYear(from text: String) -> Int? {
        ...
        if let groups = captureGroups(
            pattern: "(?i)^(?:semana|sem)\\s*(?:n[uú]mero\\s*)?(?:#\\s*)?\\d{1,2}(?:[ªaºo])?(?:\\s*(?:de|da|do|em)?\\s*('(?:\\d{2})|\\d{2}|\\d{4}))$",
            text: text
        ), groups.count >= 2 {
            return expandYear(groups[1])
        }
        ...
        if let groups = captureGroups(
            pattern: "(?i)^(?:a\\s+)?\\d{1,2}(?:[ªaºo]|\\.)\\s+semana(?:\\s+de\\s+('(?:\\d{2})|\\d{2}|\\d{4}))$",
            text: text
        ), groups.count >= 2 {
            return expandYear(groups[1])
        }
