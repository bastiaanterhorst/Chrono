// DEPrioritizeWeekNumberRefiner.swift - Prioritize German week-based parser results
import Foundation

/// Prioritizes German week parser results over conflicting results at the same index
final class DEPrioritizeWeekNumberRefiner: Refiner, @unchecked Sendable {
    func refine(context: ParsingContext, results: [ParsingResult]) -> [ParsingResult] {
        ...
        for (_, indexedResults) in groupedByIndex {
            ...
            let weekResults = indexedResults.filter { result in
                result.hasTag("DEISOWeekParser") || result.hasTag("DERelativeWeekParser")
            }
