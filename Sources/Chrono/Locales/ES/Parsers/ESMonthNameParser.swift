// ESMonthNameParser.swift - Parser for Spanish month-first and month-only expressions
import Foundation

/// Parser for Spanish month expressions like "junio", "junio 9", and "junio 9 2025".
public struct ESMonthNameParser: Parser {
    public init() {}

    public func pattern(context: ParsingContext) -> String {
        let months = PatternUtils.matchAnyPattern(ESConstants.MONTH_DICTIONARY)
        let prefix = "(?:(?:en|el)\\s+)?"
        let month = "(" + months + ")"
    ...
    public func extract(context: ParsingContext, match: TextMatch) -> Any? {
        // Avoid stealing day-first formats (e.g. "20 de febrero"), which are handled by ESMonthNameLittleEndianParser.
        let prefix = (context.text as NSString).substring(to: match.index)
        if prefix.range(of: "(?i)(?:^|\\W)\\d{1,2}\\s*(?:de|del)?\\s*$", options: .regularExpression) != nil {
            return nil
        }
        ...
        component.addTag("ESMonthNameParser")
        return component
    }
