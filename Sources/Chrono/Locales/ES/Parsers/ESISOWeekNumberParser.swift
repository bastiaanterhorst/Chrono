...
final class ESISOWeekNumberParser: AbstractParserWithWordBoundaryChecking, @unchecked Sendable {
    override func innerPattern(context: ParsingContext) -> String {
        let weekPattern = "(?i)(?:\\b(?:semana|sem)\\b\\s*(?:n[uú]mero\\s*)?(?:#\\s*)?\\d{1,2}(?:[ªaºo])?(?:\\s*(?:de|del|en)?\\s*(?:'\\d{2}|\\d{2}|\\d{4}))?)"
        let ordinalWeekPattern = "(?i)(?:\\b(?:la\\s+)?\\d{1,2}(?:[ªaºo]|\\.)\\s+semana(?:\\s+de\\s+(?:'\\d{2}|\\d{2}|\\d{4}))?)"
