// JAPrioritizeWeekNumberRefiner.swift - Prioritize Japanese week-based parser results
import Foundation

/// Prioritizes Japanese week parser results over conflicting results at the same index
final class JAPrioritizeWeekNumberRefiner: Refiner, @unchecked Sendable {
    func refine(context: ParsingContext, results: [ParsingResult]) -> [ParsingResult] {
        ...
        for (_, indexedResults) in groupedByIndex {
            ...
            let weekResults = indexedResults.filter { result in
                result.hasTag("JAISOWeekParser") || result.hasTag("JARelativeWeekParser")
            }
