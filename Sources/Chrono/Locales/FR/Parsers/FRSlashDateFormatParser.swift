// FRSlashDateFormatParser.swift - Parser for dates in European slash/dot format (DD/MM/YYYY)
import Foundation

/// Parser for French little-endian numeric dates (e.g., 31/12/2021, 31.12, 31-12-2021, 15.6).
///
/// Accepts ".", "/" and "-" as separators so dotted "dd.mm" dates (common in France) are
...
public final class FRSlashDateFormatParser: Parser {
    ...
    public func pattern(context: ParsingContext) -> String {
        return FRSlashDateFormatParser.PATTERN
    }

    /// Extracts date from slash/dot format expressions (French format: DD/MM/YYYY)
    public func extract(context: ParsingContext, match: TextMatch) -> Any? {
        let component = context.createParsingComponents()
        let calendar = Calendar.current

        // In France, the date format is DD/MM/YYYY
        ...
        component.addTag("FRSlashDateFormatParser")
        return component
    }
}
