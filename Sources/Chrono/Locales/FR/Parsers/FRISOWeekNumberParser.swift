// FRISOWeekNumberParser.swift - Parser for ISO week numbers in French text
import Foundation
...
final class FRISOWeekNumberParser: AbstractParserWithWordBoundaryChecking, @unchecked Sendable {
    ...
    private func extractWeekNumber(from text: String) -> Int? {
        ...
        if text.lowercased().contains("semaine") || text.lowercased().contains("sem") || text.lowercased().contains("w") {
