// FRRelativeWeekParser.swift - Parser for relative week expressions in French
import Foundation

/// Parser for relative week expressions in French text
final class FRRelativeWeekParser: AbstractParserWithWordBoundaryChecking, @unchecked Sendable {
    override func innerPattern(context: ParsingContext) -> String {
        let patternThis = "(?i)(?:cette\\s+semaine)"
        let patternLast = "(?i)(?:la\\s+semaine\\s+derni[eè]re|semaine\\s+derni[eè]re)"
        let patternNext = "(?i)(?:la\\s+semaine\\s+prochaine|semaine\\s+prochaine)"
        let patternWeeksAgo = "(?:(?:il\\s+y\\s+a\\s+)(\\d+)\\s+semaines?)"
        let patternInWeeks = "(?:(?:dans)\\s+(\\d+)\\s+semaines?)"
    ...
    override func innerExtract(context: ParsingContext, match: TextMatch) -> Any? {
        ...
        result.addTag("FRRelativeWeekParser")
        return result
    }
